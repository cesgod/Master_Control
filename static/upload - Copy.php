...
$pos=$_POST['position'];
$pd_ur = array();
$ti = mysqli_query($conexion, "SELECT * FROM pd_ur WHERE ID ='".$pos."' ");
echo "Pos: ".$pos;
										...
										<div class="card">
											<table class="table">
												...
												<tbody>
													...
													for ($i=0; $i < $limit; $i++) { 
														for ($j=0; $j < $pdlim; $j++) { 
															if ($pd_ur[$j]== $output["MeterName"][$i]){
																$nid=$i+1;
																#if ($nid == $pos) {
																	if (floatval($output["Porcentaje"][$i])>=85 and  floatval($output["Porcentaje"][$i])<90){
																		$class =  'class="table-warning"';
																	}elseif (floatval($output["Porcentaje"][$i])>90) {
								...
								<div class="card-body">
									<form action="up_file.php" method="POST" enctype="multipart/form-data">
										<div class="mb-3">
											...
											<input type="text" name="filename" class="form-control" placeholder="Informe" pattern="[^()/><\][\\\x22,;|]+">
										</div>
										<div class="mb-3">
											<input type="hidden" value="<?php echo $nid;?>" name="position">
